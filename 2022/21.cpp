...
public:
    Monkey(){};
    Monkey(const string _input) : input(_input)
    {
        // Parse
        auto pos = input.find(":");
        name = input.substr(0, pos);
        string operation = input.substr(pos + 2);

        pos = operation.find(' ');
        if (pos == string::npos)
        {
            number = stoi(operation);
            IKnowMyNumber = true;
        }
        else
        {
            lname = operation.substr(0, pos);
            oper = operation[pos + 1];
            rname = operation.substr(pos + 3);
        }
    }
    ...
    void calculate(map<string, Monkey> &Monkeys)
    {
        if (IKnowMyNumber)
            return;
...
private:
    ...
    friend class MonkeyMath;
};

class MonkeyMath
{
public:
    MonkeyMath(const vector<string> &_input) : input(_input)
    {
