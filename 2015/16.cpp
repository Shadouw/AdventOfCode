...
bool checkProperty ( const string &input, const string &name, const int value )
{
    auto pos = input.find(name);
    if ( string::npos != pos )
    {
        auto val = stoi(input.substr(pos+name.length()+1));

        if ( string("cats") == name || string("trees") == name )
        {
            if ( val <= value )
                return false;
        }
        else if ( string("pomeranians") == name || string("goldfish") == name )
        {
            if ( val >= value )
                return false;
        }
...
int main(int, char**) {
    ...
    for ( auto elem : input )
    {
        if ( checkProperty(elem, "children", 3) )
            if ( checkProperty(elem, "cats", 7) )
                if ( checkProperty(elem, "samoyeds", 2) )
                    if ( checkProperty(elem, "pomeranians", 3) )
                        if ( checkProperty(elem, "akitas", 0) )
                            if ( checkProperty(elem, "vizslas", 0) )
                                if ( checkProperty(elem, "goldfish", 5) )
                                    if ( checkProperty(elem, "trees", 3) )
                                        if ( checkProperty(elem, "cars", 2) )
                                            if ( checkProperty(elem, "perfumes", 1) )
                                                cout << elem << endl;
    }
