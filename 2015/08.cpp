...
string Char2Mem ( const string & str )
{
    string stripstring(str.substr(1, str.length()-2));
    string retval;
    bool escape = false;
    uint8_t hexcounter = 0;

    for ( auto c : stripstring )
    {
        if ( !escape && '\\' == c )
        {
            escape = true;
...
uint32_t calcAdditionalMem ( const string str )
{
    uint32_t additionalMem = 2;
    for ( auto c: str )
    {
        switch (c)
        {
...
int main(int, char**) {
    ...
    {
        numchar += elem.length();
        nummem  += Char2Mem(elem).length();
        resultB += calcAdditionalMem(elem);
        //cout << elem << endl;
    }

    cout << "numchar: " << numchar << endl;
    cout << "nummem: " << nummem << endl;
    cout << "Result: " << numchar - nummem << endl;
