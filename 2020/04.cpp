...
public:
    ...
    bool checkValidStrict()
    {
        if (!checkValid())
            return false;

        // byr (Birth Year) - four digits; at least 1920 and at most 2002.
        if (4 != values["byr"].length())
            return false;
        ...
        int hgt = stoi(values["hgt"]);
        string unit = values["hgt"].substr(values["hgt"].length() - 2);
        if ("cm" == unit)
        {
            if (hgt < 150 || hgt > 193)
                return false;
        ...
        for (int i = 1; i <= 6; ++i)
        {
            char curchar = values["hcl"][i];
            if (!(('0' <= curchar && curchar <= '9') || (('a' <= curchar && curchar <= 'f'))))
                return false;
        }
        ...
        if (9 != values["pid"].length())
            return false;

        for (int i = 0; i < 9; ++i)
        {
            char curchar = values["pid"][i];
...
{
public:
    PassportProcessing(const vector<string> &_input) : input(_input)
    {
    ...
    size_t countValid(bool strict = false)
    {
        size_t cnt = 0;
        for (auto e : passports)
            if (strict ? e.checkValidStrict() : e.checkValid())
                ++cnt;
...
TEST_CASE("Testdata")
{
    PassportProcessing testPassportProcessing(inputT);
    REQUIRE(2 == testPassportProcessing.countValid());

    // Part 2: Check some invalid passports
    passport pp1;
    pp1.parseString("eyr:1972 cid:100 hcl:#18171d ecl:amb hgt:170 pid:186cm iyr:2018 byr:1926");
    REQUIRE(!pp1.checkValidStrict());

    passport pp2;
    pp2.parseString("iyr:2019 hcl:#602927 eyr:1967 hgt:170cm ecl:grn pid:012533040 byr:1946");
    REQUIRE(!pp2.checkValidStrict());

    passport pp3;
    pp3.parseString("hcl:dab227 iyr:2012 ecl:brn hgt:182cm pid:021572410 eyr:2020 byr:1992 cid:277");
    REQUIRE(!pp3.checkValidStrict());

    passport pp4;
    pp4.parseString("hgt:59cm ecl:zzz eyr:2038 hcl:74454a iyr:2023 pid:3556412378 byr:2007");
    REQUIRE(!pp4.checkValidStrict());

    // Check some valid passports
    passport pp5;
    pp5.parseString("pid:087499704 hgt:74in ecl:grn iyr:2012 eyr:2030 byr:1980 hcl:#623a2f");
    REQUIRE(pp5.checkValidStrict());

    passport pp6;
    pp6.parseString("eyr:2029 ecl:blu cid:129 byr:1989 iyr:2014 pid:896056539 hcl:#a97842 hgt:165cm");
    REQUIRE(pp6.checkValidStrict());

    passport pp7;
    pp7.parseString("hcl:#888785 hgt:164cm byr:2001 iyr:2015 cid:88 pid:545766238 ecl:hzl eyr:2022");
    REQUIRE(pp7.checkValidStrict());

    passport pp8;
    pp8.parseString("iyr:2010 hgt:158cm hcl:#b6652a ecl:blu byr:1944 eyr:2021 pid:093154719");
    REQUIRE(pp8.checkValidStrict());
}

TEST_CASE("Problem")
{
    PassportProcessing testPassportProcessing(inputA);
    REQUIRE(204 == testPassportProcessing.countValid());      // Problem A
    REQUIRE(179 == testPassportProcessing.countValid(true));  // Problem B
}
