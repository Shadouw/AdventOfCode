...
public:
    ...
    void addanswer(string answer)
    {
        ...
        for (char c = 'a'; c <= 'z'; ++c)
            if (std::string::npos == answer.find(c))
                answerseveryone[c] = false;
    }
    ...
    int getAnswersEveryone()
    {
        int ret = 0;
        for ( auto elem: answerseveryone)
            if (elem.second)
                ++ret;
        return ret;
    }
...
private:
    ...
    map<char, bool> answerseveryone;
};

class CustomCustoms
{
public:
    CustomCustoms(const vector<string> &_input) : input(_input)
    {
    ...
    int getSumOfAnswers()
    {
        ...
        for (auto elem : groups)
            retval += elem.getAnswers();

        return retval;
    }

    int getSumOfAnswersEveryone()
    {
        int retval = 0;
        for (auto elem : groups)
            retval += elem.getAnswersEveryone();
...
TEST_CASE("Testdata")
{
    CustomCustoms testCustomCustoms(inputT);
    REQUIRE(11 == testCustomCustoms.getSumOfAnswers());
    REQUIRE(6 == testCustomCustoms.getSumOfAnswersEveryone());
}

TEST_CASE("CustomCustoms")
{
    CustomCustoms testCustomCustoms(inputA);
    REQUIRE(6549 == testCustomCustoms.getSumOfAnswers());
    REQUIRE(3466 == testCustomCustoms.getSumOfAnswersEveryone());
}