...
{
public:
    problem(const vector<vector<uint32_t>> &_input) : input(_input),
                                                      xsize(_input[0].size()), ysize(_input.size())
    {
    ...
    uint32_t getRisk()
    {
        ...
        for (size_t i = 1; i < ysize; ++i)
            RiskMatrix[i][0] = RiskMatrix[i - 1][0] + input[i][0];

        for (size_t j = 1; j < xsize; ++j)
            RiskMatrix[0][j] = RiskMatrix[0][j - 1] + input[0][j];
        ...
        for (size_t i = 1; i < ysize; ++i)
        {
            for (size_t j = 1; j < xsize; ++j)
            {
                uint32_t distmin = RiskMatrix[i - 1][j];
                if (RiskMatrix[i][j - 1] < distmin)
                    distmin = RiskMatrix[i][j - 1];
                /*
                if (RiskMatrix[i - 1][j - 1] + c < distmin)
                    distmin = RiskMatrix[i - 1][j - 1] + c;
*/
                RiskMatrix[i][j] = distmin + input[i][j];
            }
        }
...
private:
    const vector<vector<uint32_t>> input;
