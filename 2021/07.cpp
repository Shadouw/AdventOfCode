...
int getCostsA(int pos, const map<int, int> &crabs)
{
    int costs = 0;
    for (auto crab : crabs)
    {
        costs += abs(crab.first - pos) * crab.second;
    }
    return costs;
}

int getCostsB(int pos, const map<int, int> &crabs)
{
    int costs = 0;
    for (auto crab : crabs)
    {
        int distance = abs(crab.first - pos);
        int cost = distance * (distance+1) / 2;
        costs += cost * crab.second;
    }
    return costs;
}
...
int main(int, char **)
{
    ...
    int resultA = 0;
    int resultAPosition = 0;
    int resultACosts = INT32_MAX;
    int resultB = 0;
    int resultBPosition = 0;
    int resultBCosts = INT32_MAX;
    ...
    for (auto crab : crabs)
    {
        //cout << crab.first << "/" << crab.second << endl;
        if (crab.first > maxcrab)
            maxcrab = crab.first;
    }
    cout << "maxcrab: " << maxcrab << endl;

    for (int i = 0; i <= maxcrab; ++i)
    {
        int costsA = getCostsA(i, crabs);
        int costsB = getCostsB(i, crabs);
        if (costsA < resultACosts)
        {
            resultACosts = costsA;
            resultAPosition = i;
        }
        if (costsB < resultBCosts)
        {
            resultBCosts = costsB;
            resultBPosition = i;
        }
    }
    resultA = resultACosts;
    resultB = resultBCosts;
