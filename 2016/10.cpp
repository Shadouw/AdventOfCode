...
{
public:
    BalanceBots(const vector<string> &_input) : input(_input)
    {
    ...
    void init()
    {
        ...
        {
            int value = 0, bot = 0;
            if (sscanf(elem.c_str(), "value %d goes to bot %d", &value, &bot) == 2)
                bots[bot].insert(value);
        }
    }

    int step(const string &instruction, int valuelow = 0, int valuehigh = 0)
    {
        // Return value:
        // -2 no change
        // -1 change of data
        // other: change of data and bot compared valuelow and valuehigh
        int result = -2;

        int bot = 0, lowdest = 0, highdest = 0;
        char lowkind[20], highkind[20];
        if (sscanf(instruction.c_str(), "bot %d gives low to %s %d and high to %s %d", &bot, lowkind, &lowdest, highkind, &highdest) == 5)
        {
            // Check if bot has two microchips
            if (bots[bot].size() == 2)
            {
                int low = *(bots[bot].begin());
                int high = *(++bots[bot].begin());
                result = -1;

                if (valuelow == low && valuehigh == high)
                    result = bot;

                bots[bot].clear();

                if (strncmp("bot", lowkind, 3) == 0)
                    bots[lowdest].insert(low);
                else if (strncmp("output", lowkind, 6) == 0)
                    outputbins[lowdest].insert(low);
                else
                    cout << "Oops: " << lowkind << endl;

                if (strncmp("bot", highkind, 3) == 0)
                    bots[highdest].insert(high);
                else if (strncmp("output", highkind, 6) == 0)
                    outputbins[highdest].insert(high);
                else
                    cout << "Oops: " << highkind << endl;
            }
        }
    ...
    long getResultA(int valuelow, int valuehigh)
    {
        long resultA = -1;
        ...
        while (0 > resultA)
        {
            for (auto elem : input)
            {
                long temp = step(elem, valuelow, valuehigh);
                if (0 <= temp)
                    resultA = temp;
            }
        }
    ...
    long getResultB()
    {
        ...
        long resultB = *(outputbins[0].begin()) * *(outputbins[1].begin()) * *(outputbins[2].begin());
...
private:
    ...
    map<int, set<int>> outputbins;
    map<int, set<int>> bots;
};

TEST_CASE("Testdata")
{
    BalanceBots BalanceBotsData(inputTestdata);
    REQUIRE(2 == BalanceBotsData.getResultA(2, 5));
    REQUIRE(30 == BalanceBotsData.getResultB());
}

TEST_CASE("BalanceBots")
{
    BalanceBots BalanceBotsData(inputData);
    REQUIRE(86 == BalanceBotsData.getResultA(17, 61));
    REQUIRE(22847 == BalanceBotsData.getResultB());
}