...
public:
    struct room{
        ...
        int sectorid;
        string checksum;
        room (string inputstring)
        {
            size_t posbracket = inputstring.rfind('[');
            size_t posdash = inputstring.rfind('-');
            checksum = inputstring.substr(posbracket + 1, inputstring.length() - posbracket - 2);
            sectorid = stoi(inputstring.substr(posdash + 1));
            name = inputstring.substr(0, posdash);
        }
        string calculatechecksum()
        {
            map<char, int> lettersinname;
            string cs;
            ...
            // sort lettersinname by value
            multimap<int, char> lettersinnameordered;
            for ( auto p : lettersinname )
                lettersinnameordered.insert(std::pair<int, char>(p.second*-1,p.first));

            for ( auto it = lettersinnameordered.begin(); it != lettersinnameordered.end(); ++it)
                cs += it->second;

            return cs.substr(0,5);
        }
    ...
    problem(const vector<string> &_input) : input(_input)
    {
        ...
        for ( auto elem : input )
            listofrooms.push_back(room(elem));
    }
    ...
    void removeDecoy()
    {
        for (auto it = listofrooms.begin(); it != listofrooms.end();)
        {
            if ( !it->checkifreal() )
                it = listofrooms.erase(it);
            else
                ++it;
        }
    }

    unsigned long getSumOfSectors() 
    {
        unsigned long retval = 0;
        for (auto elem : listofrooms)
        {
            retval += elem.sectorid;
        }
        return retval;
    }
...
TEST_CASE ( "Parse Room" )
{
    problem::room myroom("aaaaa-bbb-z-y-x-123[abxyz]");
    REQUIRE ( "aaaaa-bbb-z-y-x" == myroom.name );
    REQUIRE ( 123 == myroom.sectorid );
    REQUIRE ( "abxyz" == myroom.checksum );
    REQUIRE ( "abxyz" == myroom.calculatechecksum() );

    REQUIRE(true == myroom.checkifreal());

    problem::room myroom2("a-b-c-d-e-f-g-h-987[abcde]");
    REQUIRE(true == myroom2.checkifreal());

    problem::room myroom3("not-a-real-room-404[oarel]");
    REQUIRE(true == myroom3.checkifreal());

    problem::room myroom4("totally-real-room-200[decoy]");
    REQUIRE(false == myroom4.checkifreal());

    problem::room decryptroom("qzmt-zixmtkozy-ivhz-343[]");
    REQUIRE(343 == decryptroom.sectorid);
    REQUIRE("very encrypted name" == decryptroom.decrypt());
}

TEST_CASE ( "Testdata" )
{
    problem testproblem(inputT);
    testproblem.removeDecoy();

    REQUIRE(1514 == testproblem.getSumOfSectors());
}

TEST_CASE ( "First Problem" )
{
    problem testproblem(inputA);
    testproblem.removeDecoy();

    // First Solution:
    REQUIRE(137896 == testproblem.getSumOfSectors());

    // Second Solution:
    for ( auto elem: testproblem.listofrooms )
    {
        if ("northpole object storage" == elem.decrypt() )
            REQUIRE(501 == elem.sectorid);
    }

    //testproblem.printDecoded();
}