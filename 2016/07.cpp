...
{
public:
    IP(const string _input) : input(_input)
    {
    }
    ...
    static string ABA2BAB(string aba)
    {
        assert(3 == aba.size());
        assert(aba[0]==aba[2]);
        return aba.substr(1,2)+aba.substr(1,1);
    }
    ...
    bool getSSLSupport()
    {
        ...
        for (int i = 0; i < input.size() - 2; ++i)
        {
            ...
            {
                string substr(input.substr(i, 3));
                if (0 != bracketcount)
                    ++ABA[substr];
                else
                    ++BAB[substr];
...
{
public:
    IPv7(const vector<string> &_input) : input(_input)
    {
        cout << "Size of Input: " << input.size() << endl;

        // Parse data
        for (auto elem : input)
            IPs.push_back(IP(elem));
    }
    ...
    long getTLSSupport()
    {
        ...
        cout << "TLSSupport: " << TLSSupport << endl;
        return TLSSupport;
    }
    long getSSLSupport()
    {
        ...
        cout << "SSLSupport: " << SSLSupport << endl;
        return SSLSupport;
    }
...
TEST_CASE("Testdata")
{
    IP ip1("abba[mnop]qrst");
    REQUIRE(true == ip1.getTLSSupport());
    IP ip2("abcd[bddb]xyyx");
    REQUIRE(false == ip2.getTLSSupport());
    IP ip3("aaaa[qwer]tyui");
    REQUIRE(false == ip3.getTLSSupport());
    IP ip4("ioxxoj[asdfgh]zxcvbn");
    REQUIRE(true == ip4.getTLSSupport());

    IPv7 IPv7Data(inputTestdata);

    REQUIRE(2 == IPv7Data.getTLSSupport());
}

TEST_CASE("Testdata2")
{
    REQUIRE("aba" == IP::ABA2BAB("bab"));

    IP ip1("aba[bab]xyz");
    REQUIRE(true == ip1.getSSLSupport());
    IP ip2("xyx[xyx]xyx");
    REQUIRE(false == ip2.getSSLSupport());
    IP ip3("aaa[kek]eke");
    REQUIRE(true == ip3.getSSLSupport());
    IP ip4("zazbz[bzb]cdb");
    REQUIRE(true == ip4.getSSLSupport());

    IPv7 IPv7Data(inputTestdata2);

    REQUIRE(3 == IPv7Data.getSSLSupport());
}

TEST_CASE("IPv7")
{
    IPv7 IPv7Data(inputData);
    REQUIRE(110 == IPv7Data.getTLSSupport());
    REQUIRE(242 == IPv7Data.getSSLSupport());
}