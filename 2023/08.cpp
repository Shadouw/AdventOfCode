...
struct nodeStatus {
    ...
    long ip;
    long long firsthit;
    long long hitdistance;
};

class HauntedWasteland {
public:
    HauntedWasteland(const vector<string>& _input)
        : input(_input)
    {
        ...
        for (auto elem : input) {
            char node[4], nodel[4], noder[4];
            int result = sscanf(elem.c_str(), "%3s = (%3s, %3s)", node, nodel, noder);

            if (3 == result)
                nodes[node] = pair<string, string>(nodel, noder);
        }
    }
    ...
    long getResultA()
    {
        ...
        while ("ZZZ" != currentNode) {
            ...
            currentNode = ('L' == instructions[ip] ? nodes[currentNode].first : nodes[currentNode].second);
    ...
    long long getResultB()
    {
        ...
        for (auto m : nodes)
            if ('A' == m.first[2])
                currentNodes.push_back(m.first);

        // Calculate Periods
        for (auto currentNode : currentNodes) {
            long ip = 0; //Instruction Pointer
            long long stepcounter = 0;
            const string startnode = currentNode;
...
TEST_CASE("Testdata")
{
    HauntedWasteland HauntedWastelandData(inputTestdata);
    REQUIRE(2 == HauntedWastelandData.getResultA());
}

TEST_CASE("Testdata2")
{
    HauntedWasteland HauntedWastelandData(inputTestdata2);
    REQUIRE(6 == HauntedWastelandData.getResultA());
}

TEST_CASE("Testdata3")
{
    HauntedWasteland HauntedWastelandData(inputTestdata3);
    REQUIRE(6 == HauntedWastelandData.getResultB());
}

TEST_CASE("HauntedWasteland")
{
    HauntedWasteland HauntedWastelandData(inputData);
    REQUIRE(17287 == HauntedWastelandData.getResultA());
    REQUIRE(18625484023687 == HauntedWastelandData.getResultB());
}