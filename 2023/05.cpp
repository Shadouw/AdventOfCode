...
struct mapentry {
    mapentry(long long d, long long s, long long r)
        : dest(d)
        , source(s)
        , range(r)
    {
...
private:
    ...
    friend class IfYouGiveASeedAFertilizer;
};

class IfYouGiveASeedAFertilizer {
public:
    IfYouGiveASeedAFertilizer(const vector<string>& _input)
        : input(_input)
    {
        cout << "Size of Input: " << input.size() << endl;

        // First line contains seeds:
        for (auto e : stringtovector(input[0].substr(input[0].find(':') + 1), ' '))
            seeds.push_back(stoll(e));
