...
public:
        ...
        : input(_input)
    {
        ...
        if (string::npos == pos) {
            ...
            condvar = input[0];
            cond = input[1];
            value = stol(input.substr(2));
            targetstate = input.substr(pos + 1);
        }
    }
...
class workflow {
public:
    workflow(string _input)
        : input(_input)
    {
        // Split name and conditions, remove brackets
        auto pos = input.find('{');
        name = input.substr(0, pos);

        vector<string> tcond = stringtovector(input.substr(pos + 1, input.size() - pos - 2), ',');
        for (auto t : tcond)
            conditions.push_back(condition(t));
    }
...
private:
    ...
    friend class Aplenty;
    friend class part;
};

class part {
public:
    part(const string _input)
        : input(_input)
    {
        // Remove brackets {}
        string part = input.substr(1, input.size() - 2);
        vector<string> tokens = stringtovector(part, ',');
        for (auto t : tokens) {
            long value = stol(t.substr(2));
            switch (t[0]) {
            case 'x':
                x = value;
                break;
     ...
     * @brief
     *
     * @return true Final state (A or R) reached.
     * @return false
     */
    bool process(const workflow& w)
    {
        for (auto c : w.conditions) {
            if ('=' == c.cond) {
                ...
                case 's':
                    ...
                    break;
                default:
                    cout << "Unexpected condition variable: " << c.condvar << endl;
                    break;
                }

                if (('<' == c.cond && value < c.value) || ('>' == c.cond && value > c.value)) {
                    state = c.targetstate;
                    break;
                }
            }
        }
        return ("R"==state || "A"==state);
    }
...
class Aplenty {
public:
    Aplenty(const vector<string>& _input)
        : input(_input)
    {
        ...
        for (auto elem : input) {
            if ("" == elem)
                ...
                workflow t = workflow(elem);
                workflows[t.name] = workflow(elem);
            } else
    ...
    {
        for (auto& p : parts) {
            while (!p.process(workflows[p.state])) { };
        }
    }
...
TEST_CASE("Testdata")
{
    Aplenty AplentyData(inputTestdata);
    REQUIRE(19114 == AplentyData.getResultA());
    REQUIRE(0 == AplentyData.getResultB());
}

TEST_CASE("Aplenty")
{
    Aplenty AplentyData(inputData);
    REQUIRE(352052 == AplentyData.getResultA());
    REQUIRE(0 == AplentyData.getResultB());
}